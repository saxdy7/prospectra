# Prospectra â€” Frontend Route Inventory

Every route in the app, what it's for, its primary action, which states it
supports, and â€” critically â€” what on the page is real (backed by
...
- Auth/onboarding gate: `useWorkspace()` â†’ no session â†’ `/signin`; no
  completed onboarding â†’ `/onboarding`. Renders `PageSkeleton` while
  resolving.
- Shell: `src/app/app/layout.tsx` â†’ `AppShell` (sidebar, compact top bar,
  `ToastProvider`). Individual pages render only their content.
- Keyboard: every control is a real `<button>`/`<input>`/`<a>`/`<Link>` â€”
  no `div onClick`. Drawers and dialogs trap focus and close on Escape.
  Global focus ring: 2px `--lp-blue-mid`, 3px offset.
- Responsive: sidebar â†’ drawer below 900px; tables scroll horizontally
  rather than compress; touch targets â‰¥44px via `.pa-btn`/`.pa-icon-btn`
  sizing.
- Reduced motion: the global `.pa` `prefers-reduced-motion` block collapses
  ...
  drawer/dialog/toast entrances added in this milestone.
...
|---|---|---|---|---|
| `/` | Marketing landing page | Start Prospecting Free â†’ `/signup` | â€” | Unchanged this milestone |
...
| `/onboarding` | 5-step workspace setup | Continue | per-step validation, saved-indicator, finish | Unchanged this milestone; already dark. See Â§3 of the UI spec |
...
activity (only real onboarding-derived events â€” never invented), table/row
counts. The "Calling setup" card only renders when onboarding recorded
interest in calling.
...
|---|---|---|---|
| `/app/find-leads` | Hub â€” five cards, one per search kind | Pick a kind | â€” |
...
carrying a `DemoTag`. No search ever produces real results â€” a provider
...
| `/app/tables/[tableId]` | Reactive grid: Grid / Views / Activity tabs | Add rows | empty, loading (skeleton rows), populated, not-found (invalid id â†’ friendly message + back link) |
| `/app/imports` | Import history | New import | empty, populated |
| `/app/imports/new` | CSV upload â†’ column mapping â†’ validation preview â†’ commit | Import | pick, map, done |
...
and reports real counts â€” nothing is fabricated. Grid supports real
...
| `/app/workflows` | Templates + your workflows + visual canvas | New workflow | `WorkflowCanvas`: real drag-to-move nodes, click-to-connect edges, editable labels. **Nothing executes** â€” this is explicitly a frontend-only builder |
| `/app/mcp` | Client permissions, credit defaults, function visibility | Copy endpoint | Endpoint URL is real (workspace-derived); client "connections" are honestly `not_connected` â€” authorizing a real client needs account-level API keys, which don't exist yet |
...
| `/app/campaigns/new` | 4-step wizard: Audience â†’ Channel â†’ Draft â†’ Review | Save draft | Per-step validation; accepts `?audience=` to preselect |
| `/app/campaigns/[campaignId]` | Summary, sequence, members, honestly-empty performance | Send (â†’ confirms no provider is connected) | not-found |
...
demo-only state change** â€” it never sends anything. Performance numbers
...
|---|---|---|
| `/app/voice-agents` | Directory | â€” |
| `/app/voice-agents/new` | 6-step create flow | Name â†’ Role â†’ Languages & voice â†’ First message â†’ Tools â†’ Preview |
...
| `/app/phone-numbers` | Number directory | Honest empty state â€” purchasing needs a telephony provider |
...
(`lib/mock-data/call-logs.ts` â€” this agent has placed zero real calls, the
...
**"Simulation mode"** explicitly â€” it is a canned back-and-forth, not a
...
| Route | Purpose | Notes |
|---|---|---|
| `/app/analytics` | Overview, phone performance, call patterns, daily stats, agent leaderboard | Top block ("Workspace â€” live counts") is 100% real derived from `WorkspaceData`. Everything below a `DemoTag`-labelled divider is `lib/mock-data/analytics.ts` |
| `/app/activity` | Filterable event feed | 100% real â€” every entry comes from `logActivity()` calls made by other pages when the user does something (create table, save search, draft campaign, â€¦). Nothing pre-seeded |
| `/app/integrations` | Directory across CRM/enrichment/LLM/email/telephony/storage | Every item `not_connected` or `coming_soon`; "Connect" is disabled on every card |
| `/app/webhooks` | Endpoints + delivery log | Endpoints are real, persisted `WebhookEndpoint` records with a real (random) secret ref. Delivery log is an honest empty state â€” no event has ever fired one |
| `/app/settings` | General / Members / Branding / Security / Credits / Notifications tabs | General (name, logo, team size, destructive reset) is fully real and was carried over unchanged. Members/Branding/Security are honest "coming soon" panels. Credits shows the real static 500-credit starting grant. Notification toggles persist locally but state plainly that delivery needs a connected email address |
| `/app/help` | Searchable guide directory | Static content; guide links are placeholders (`href="#"`) since no docs site exists yet â€” intentionally not styled as real pages that 404 |

---

## Route count

**42 total** â€” 5 public/onboarding + 1 home + 6 find-leads + 4 tables/imports
+ 5 research/automation + 5 audiences/campaigns + 12 voice + 6
analytics/operations + `/auth/callback` (server route, not a page).

## Verification performed

- `npx tsc --noEmit` â€” clean.
- `npm run lint` â€” clean (0 errors after converting every internal
  cross-route `<a href="/app/...">` to `next/link`'s `Link`, per
  `@next/next/no-html-link-for-pages`).
- `npm run build` â€” all 42 routes compiled; 38 static (`â—‹`), 4 dynamic
  (`Æ’`: `/app/tables/[tableId]`, `/app/audiences/[audienceId]`,
  `/app/campaigns/[campaignId]`, `/app/voice-agents/[agentId]` +
  `/app/voice-agents/[agentId]/test`, `/auth/callback`).
- Live `curl` sweep of every route on `npm run dev`, including the dynamic
  ...
  not-found empty state renders rather than crashing) â€” all 200.
- Not verified from this environment (no browser available): actual visual
  rendering, mobile viewport behaviour, and screen-reader announcement â€”
  these were built to the documented rules (Â§8/Â§9 of the UI spec) but a
  human pass with a real browser is the honest remaining step. See the
  final report for the full list.


| `/app/invoices` | Billing history | | empty (no provider) |
